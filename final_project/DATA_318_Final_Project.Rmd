...
```{r}
Forex_DF <- read.csv("https://proxy.hxlstandard.org/data/e6e27d/download/ECB_FX_USD-base.csv",stringsAsFactors=TRUE)
...
Forex_DF$EUR <- as.numeric(as.character(Forex_DF$EUR))
...
Forex_EUR_Leads_DF <- dplyr::select(Forex_Lead2_DF,ends_with(c("Date","EUR",".lead",".lead2"))) %>% dplyr::select(!contains(c("EUR.lead","EUR.lead2","BGN","DKK","CZK","SEK","CHF","HUF","GBP","PLN","RON","ISK","NOK","HRK","CAD","AUD","NZD")))
...
## Data Visualization
```{r}
ggplot(Forex_DF_asc,mapping=aes(x=Date,y=EUR,group=1)) +
  geom_line() +
  labs(x="1999-2022",y="EUR/USD Exchange Rate") +
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
ggplot(Forex_DF_asc,mapping=aes(x=Date,y=DKK,group=1)) +
  geom_line() +
  labs(x="1999-2022",y="DKK/USD Exchange Rate") +
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) #Trend is nearly identical to EUR
cor(Forex_DF$EUR,Forex_DF$DKK)
ggplot(Forex_DF_asc,mapping=aes(x=Date,y=AUD,group=1)) +
  geom_line() +
  labs(x="1999-2022",y="AUD/USD Exchange Rate") +
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) #Trend is nearly identical to EUR
cor(Forex_DF$EUR,Forex_DF$AUD)
ggplot(Forex_DF_asc,mapping=aes(x=Date,y=JPY,group=1)) +
  geom_line() +
  labs(x="1999-2022",y="JPY/USD Exchange Rate") +
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
cor(Forex_DF$EUR,Forex_DF$JPY)
ggplot(Forex_DF_asc,mapping=aes(x=Date,y=CNY,group=1)) +
  geom_line() +
  labs(x="1999-2022",y="CNY/USD Exchange Rate") +
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
cor(Forex_DF$EUR,Forex_DF$CNY)
ggplot(Forex_DF_asc,mapping=aes(x=Date,y=KRW,group=1)) +
  geom_line() +
  labs(x="1999-2022",y="KRW/USD Exchange Rate") +
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
...
```
## PCR Model using optimal BIC subset variables
```{r}
formula
#could only get it to work when copying the text of 'formula' rather than 'formula' itself
eur_pcr <- pcr(EUR~JPY.lead+CNY.lead+ILS.lead+KRW.lead+MYR.lead+SGD.lead+ZAR.lead+HKD.lead2+IDR.lead2+INR.lead2+MXN.lead2+MYR.lead2+PHP.lead2+THB.lead2, data = train_forex, scale = TRUE, validation = "CV")
...
```
## Bagged Trees using optimal BIC subset variables
```{r}
formula
#could only get it to work when copying the text of 'formula' rather than 'formula' itself
bag.mod <- bagging(
  formula =EUR~JPY.lead+CNY.lead+ILS.lead+KRW.lead+MYR.lead+SGD.lead+ZAR.lead+HKD.lead2+IDR.lead2+INR.lead2+MXN.lead2+MYR.lead2+PHP.lead2+THB.lead2,
  data = train_forex,
  nbagg = 150,
  coob = TRUE,
...
```
## Boosted Trees using optimal BIC subset variables
```{r}
formula
#could only get it to work when copying the text of 'formula' rather than 'formula' itself
eur_boost<-gbm(EUR~JPY.lead+CNY.lead+ILS.lead+KRW.lead+MYR.lead+SGD.lead+ZAR.lead+HKD.lead2+IDR.lead2+INR.lead2+MXN.lead2+MYR.lead2+PHP.lead2+THB.lead2, data=train_forex, 
